...
*Bug I didn't have time to sort out:*
* Assessment asked for a 10 day forecast, but right now defaults to 7 days since I don't have `&forecast_days=10` set for the API queries. It doesn't appear to be a trivial fix since marine datasets (wave height) don't go out to 10 days forecast, so that was causing some issues with syncing when datasets didn't end at the same time. 
...
The captain wanting a mobile version would be MEDIUM importance and LOW effort (it almost already works on mobile, I checked).
